...
We will see a quick setup to configure the ``tool_karabiner`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_karabiner`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_karabiner`` formulas under ``/srv/salt/tool_karabiner/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_karabiner/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_karabiner/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_karabiner/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_karabiner/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_karabiner`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_karabiner`` formulas under ``/srv/salt/tool_karabiner/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_karabiner/parameters/roles

We will define 2 roles:

- ``tool_karabiner/server``
- ``tool_karabiner/client``

We create a configuration for the role ``tool_karabiner/server`` in ``/srv/salt/tool_karabiner/parameters/roles/tool_karabiner/server.yaml``:
...
We create another configuration for the role ``tool_karabiner/client`` in ``/srv/salt/tool_karabiner/parameters/roles/tool_karabiner/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_karabiner/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_karabiner/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_karabiner/parameters/dns:domain/  '/srv/salt/tool_karabiner/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-karabiner-as-json:
      test.nop:
        - name: {{ karabiner | json }}
...
.. code-block:: sls
    ...
    karabiner-config-file-file-managed:
      file.managed:
        - name: {{ karabiner.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='karabiner-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ karabiner.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``karabiner`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ karabiner['winner'] }}
